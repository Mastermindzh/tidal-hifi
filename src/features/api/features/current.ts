...
export const addCurrentInfo = (expressApp: Router) => {
   ...
   *         albumArt: "/path/to/downloaded/current.jpg"
   *         favorite: true
   *         player:
   *           status: "playing"
   *           shuffle: true
...
export const getCurrentImage = (req: Request, res: Response) => {
  ...
  const imagePath = mediaInfo.albumArt || mediaInfo.image || mediaInfo.icon;
  ...
  // Logger.log("image?", { albumArt: mediaInfo.albumArt, image: mediaInfo.image, using: imagePath });
